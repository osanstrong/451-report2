...
Over the 5-minute (300 second) Source period, we observed gross count $n_G$ of 3469, with an uncertainty of $\sigma_G=\sqrt{n_G}=58.90$.
...
Over the 10-minute (600 second) Source-free period, we observed a background count $n_B$ of 386, with an uncertainty of $\sigma_B=\sqrt{n_B}=19.647$. 
...
As expected, the Poisson distribution appears to appropriately model the count data series, especially for the series with the higher number of trials, but notably the other distributions produce believable fits. The series additionally both have similar variances to the number of counts, reminiscent of the Poisson distribution, which only has one degree of freedom and for which variance corresponds to average counts.\\
...
While a net count rate was observed within $10.920\pm0.196 \mathrm{s^{-1}}$, the calibrated activity of the beta source is 1.0 microCurie\cite{npre_lab_2_manual_2025}, equivalent to $3.7\times10^{10}\mathrm{s^{-1}}\times\mathrm{\frac{1Ci}{10^6\mu Ci}}=37,000\mathrm{s^{-1}}$, dramatically higher.\\
...
A series of 20 radiation counts were taken by a Geiger-Muller detector of a beta source over 30 seconds each, followed by a series of 200 5-second counts, and modeled by Poisson, binomial, and normal models.
